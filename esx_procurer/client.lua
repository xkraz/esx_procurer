...
exports(
    ...
    function()
        missionStarted = true
        TriggerServerEvent('procurer:startWorking')
    end
)
...
RegisterNetEvent('procurer:startGuarding')
AddEventHandler(
    'procurer:startGuarding',
    function()
        spawnHookers()
        ...
        if IsPedDeadOrDying(SpawnedPed) == false then
            ...
            while workingTime > 0 do
                ...
                if workingTime < 1 then
                    ...
                    FreezeEntityPosition(SpawnedHooker, false)
                    resetCustomer()
                    TriggerServerEvent('procurer:pay', Customer.modelType[type].name)
                end
            end
        else
            ESX.ShowNotification('Le ~g~client~w~ est ~r~mort~w~!')
...
function resetWork()
    TriggerServerEvent('procurer:workDone')
    SetPedAsNoLongerNeeded(SpawnedPed)
    SetModelAsNoLongerNeeded(SpawnedPed)
